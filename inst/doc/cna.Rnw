...
\bibitem[{Amb\"uhl and Baumgartner(2021)}]{cnaOptRef}
Amb\"uhl M, Baumgartner M (2021).
\newblock \emph{\pkg{cnaOpt}: Optimizing Consistency and Coverage in
  Configurational Causal Modeling.}
\newblock \textsf{R} Package Version 0.5.0.
...
\bibitem[{Baumgartner and Falk(2021)}]{CNA_LR}
Baumgartner M, Falk C (2021).
\newblock \enquote{Configurational Causal Modeling and Logic Regression.}
\newblock \emph{Multivariate Behavioral Research}.
...
\newblock \emph{\pkg{SetMethods}: Functions for Set-Theoretic Multi-Method Research
  and Advanced QCA}.
...
\newblock \emph{\pkg{frscore}: Functions for Calculating Fit-Robustness of
  {CNA}-solutions.}
\newblock \textsf{R} Package Version 0.1.0.
