...
condList <- function(x, ct = full.ct(x), ..., 
                     verbose = TRUE){
  if (verbose && missing(ct))
    message("Argument ", dQuote("ct"), " in condList() is missing;\n",
            "a complete configTable generated by full.ct() is used.")
  UseMethod("condList")
}

condition <- function(x, ct = full.ct(x), ..., verbose = TRUE){
  cl <- match.call()
  cl[[1]] <- as.name("condList")
  eval.parent(cl)
}
...
condList.character <- function(x, ct = full.ct(x), 
                               measures = c("standard consistency", "standard coverage"), 
                               type, add.data = FALSE, force.bool = FALSE, 
                               rm.parentheses = FALSE, ..., verbose = TRUE){
  ...
  cti <- ctInfo(ct)
  out <- condFromCti(x = x, cti = cti, ct = ct, type = cti$type, 
                     add.data = add.data, force.bool = force.bool, 
                     rm.parentheses = rm.parentheses, measures = measures, ...)
  # NOTES: 
  # * attributes "type", "n", "cases" are present both at condList and cond level!!
  out
}
...
condFromCti <- function(x, cti, ct = cti2ct(cti), type = cti$type, 
  ...
  if (any(stdAtomic <- (condTypes == "stdAtomic"))){
    ...
    infoUpdate <- if (!force.bool){
      getInfo_asf(qc, n.cases, conCovDef = conCovDef)
    } else {
  ...
  if (any(stdComplex <- (condTypes == "stdComplex"))){
    qc <- qcond_csf(x[stdComplex], cti$scores, conCovDef = conCovDef, 
                    force.bool = force.bool, freqs = n.cases)
  ...
  if (any(bool <- (condTypes == "boolean"))){
    ...
    .newInvalid <- which(bool)[vapply(out[bool], inherits, "invalidCond", 
                                      FUN.VALUE = logical(1))]
    if (any(.newInvalid)){
  ...
  if (any(complx <- (condTypes == "complex"))){
    ...
    info <- updateInfo(info, complx,
                       getInfo_customCsf(out[i_x], lengths(out[i_x]), 
                                         n.cases, conCovDef = conCovDef))
  }
...
# condList method for class condTbl
condList.condTbl <- function(x, ct = full.ct(x), 
    measures = attr(x, "measures"), ..., verbose = TRUE){
  if (!("condition" %in% names(x)))
      stop("Required column ", dQuote("condition"), " is missing.")
  out <- condList.character(x[["condition"]], ct = ct, measures = measures, ...)
  out
}
